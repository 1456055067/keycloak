...
pub struct GrantResult {
    ...
    /// User session ID (if created/updated).
    pub user_session_id: Option<Uuid>,

    /// Client session ID (if created/updated).
    pub client_session_id: Option<Uuid>,
}

/// Handler for `authorization_code` grant.
pub struct AuthorizationCodeGrant<S: AuthCodeStore> {
    code_store: Arc<S>,
}

impl<S: AuthCodeStore> AuthorizationCodeGrant<S> {
    /// Creates a new authorization code grant handler.
    #[allow(clippy::missing_const_for_fn)] // Can't be const: moves Arc
    pub fn new(code_store: Arc<S>) -> Self {
        Self { code_store }
    }

    /// Handles the authorization code grant.
    pub async fn handle(&self, ctx: &GrantContext<'_>) -> OidcResult<GrantResult> {
        ...
        let token_response = ctx
            ...
            .create_token_response(
                ...
                &stored_code.scope,
                stored_code.user_session_id.map(|id| id.to_string()).as_deref(),
        ...
        Ok(GrantResult {
            token_response,
            user_session_id: stored_code.user_session_id,
            client_session_id: stored_code.client_session_id,
        })
    }
...
impl ClientCredentialsGrant {
    ...
    pub fn handle(&self, ctx: &GrantContext<'_>) -> OidcResult<GrantResult> {
...
impl RefreshTokenGrant {
    ...
    pub fn handle(&self, ctx: &GrantContext<'_>) -> OidcResult<GrantResult> {
        ...
        let scope = ctx
            ...
            .scope
            .as_deref()
            .unwrap_or_else(|| claims.scope.as_deref().unwrap_or("openid"));

        // TODO: Validate requested scope is subset of original scope
        ...
        Ok(GrantResult {
            token_response,
            user_session_id: claims.sid.and_then(|s| Uuid::parse_str(&s).ok()),
            client_session_id: None,
        })
    }
}
...
impl<U: UserAuthenticator> PasswordGrant<U> {
    ...
    pub async fn handle(&self, ctx: &GrantContext<'_>) -> OidcResult<GrantResult> {
        ...
        Ok(GrantResult {
            token_response,
            user_session_id: None, // TODO: Create session
            client_session_id: None,
        })
    }
}
